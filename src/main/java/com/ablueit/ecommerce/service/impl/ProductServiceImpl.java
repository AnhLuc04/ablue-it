...
public class ProductServiceImpl implements ProductService {
    ...
    public String updateVariationProduct(ProductRequest request) throws IOException {
        ...
        Product product = productRepository.findBySku(request.sku())
                .orElseThrow(() -> new ResourceNotFoundException("Product not found with SKU: " + request.sku()));
        ...
        product.setPrice(request.regularPrice());
        product.setSalePrice(request.salePrice());
        product.setStockQuantity(request.stockQuantity());
        product.setStockStatus(StockStatus.valueOf(request.stockStatus()));
        product.setStatus(ProductStatus.PUBLISHED);

        // ✅ Cập nhật danh mục
        Categories category = getCategoryByName(request.category());
        product.setCategories(List.of(category));

        // ✅ Cập nhật lại ảnh nếu có gửi mới
        List<MultipartFile> newImages = new ArrayList<>();
        List<ImageType> imageTypes = new ArrayList<>();

        if (!request.primaryImage().isEmpty()) {
            newImages.add(request.primaryImage());
            imageTypes.add(ImageType.PRIMARY);
        }

        if (!request.sizeGuideImage().isEmpty()) {
            newImages.add(request.sizeGuideImage());
            imageTypes.add(ImageType.SIZE_GUIDE);
        }

        if (request.galleryImages() != null && !request.galleryImages().isEmpty()) {
            for (MultipartFile galleryImage : request.galleryImages()) {
        ...
        if (!newImages.isEmpty()) {
            // Xoá ảnh cũ (tuỳ chọn nếu bạn muốn xoá)
            productImageRepository.deleteAll(product.getProductImages());

            // Lưu ảnh mới
            List<ProductImage> updatedImages = saveImageFiles(product.getSku(), newImages, imageTypes, product);
            product.setProductImages(updatedImages);
        }

        // ✅ Xoá toàn bộ variations cũ và tạo lại (đơn giản nhất)
        variationRepository.deleteAll(product.getVariations());
        ...
        List<Variation> variations = variationFromRequest.stream().map(variationRequest -> {
            ...
            List<VariationAttribute> variationAttributes = variationRequest.attributes().stream().map(attributeRequest -> {
                Attribute attribute = getAttributeByNameOrElseCreateNew(attributeRequest.name());
                attributeRepository.save(attribute);

                AttributeTerm term = getAttributeTermByNameOrElseCreateNew(attributeRequest.term(), attribute);
                attributeTermRepository.save(term);
    ...
    public ProductRequest mapToRequest(Product product) {
        List<VariationRequest> variationRequests = product.getVariations().stream().map(variation -> {
            List<VariationRequest.AttributeRequest> attributes = variation.getAttributes().stream()
                    .map(va -> new VariationRequest.AttributeRequest(
                            va.getAttributeTerm().getAttribute().getName(),
                            va.getAttributeTerm().getName()
                    )).toList();

            return new VariationRequest(attributes, variation.getPrice(), variation.getStockQuantity());
        }).toList();

        String category = product.getCategories().isEmpty() ? null : product.getCategories().get(0).getName();
        ...
        return new ProductRequest(
                ...
                variationRequests,
                null, // primaryImage: không truyền khi GET
                null, // sizeGuideImage
                null  // galleryImages
        );
    }
